 ...
 <style type="text/css">
      ...
      <table class="table table-striped table-advance table-hover">
       	...
       	<?php if($mode == "read"):?>
               ...
               <thead>
           			<tr><th>Title</th><th>Owner</th><th>Post</th><th>Image</th><th>Date created</th><th>Action</th></th></tr>        			
		 	...
		 	<?php if(isset($blogs) && is_array($blogs)):?>  
			 	...
			 	<tr style="position:relative;"><td><?=$blog->title?></td><td><a href="<?=ROOT?>profile/<?=$blog->user_url?>"><?=$blog->user_data->name?></a></td><td><?=$blog->post?></td><td><img src="<?=ROOT.$blog->image?>" style="width:150px;" /></td></td><td><?=date("jS M Y H:i a", strtotime($blog->date))?></td>
	      ...
	      <div class="status alert alert-success" style="">The blog was post successfully</div>
	      	 <a href="<?=ROOT?>admin/blogs">
                 <input type="button" class="btn btn-success pull-right" value="Back to post"/>
               </a>
	      ...
	      <?php elseif($mode == "delete" && is_object($blogs)):?> 
               ...
               <tbody>
	      	<tr style="position:relative;"><td><?=$blogs->title?></td><td><a href="<?=ROOT?>profile/<?=$blogs->user_url?>"><?=$blogs->user_data->name?></a></td><td><?=$blogs->post?></td><td><img src="<?=ROOT.$blogs->image?>" style="width:150px;" /></td></td><td><?=date("jS M Y H:i a", strtotime($blogs->date))?></td>
